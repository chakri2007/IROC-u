...
class TCPCommNode(Node):
    def __init__(self):
        ...
        # Listen thread
        listen_thread = threading.Thread(target=self.listen_to_gcs, daemon=True)
        listen_thread.start()

# Inside drone_status_callback:
    def drone_status_callback(self, msg):
        ...
        if status == "LANDED":
            ...
            self.send_command("STOP_CHARGING")
        elif status == "IDLE":
    ...
    def listen_to_gcs(self):
        while True:
            try:
                ...
                message = self.client_socket.recv(length).decode('utf-8').strip()

                self.get_logger().info(f'GCS Response: {message}')
                ...
                elif "Charging started" in message or "Charging with" in message:
                    ros_msg.data = "CHARGING"

                elif "Charging stopped" in message or "stopped" in message.lower():
                ...
                self.get_logger().info(f'Published to /gcs/status: {ros_msg.data}')
