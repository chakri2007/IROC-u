...
class AutonomousPrecisionLanding(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("climb_speed", 0.5)       # m/s upward during takeoff
    ...
    def run_mission(self):
        ...
        self.get_logger().info("Pre-streaming setpoints for OFFBOARD …")
        ...
        # ---- 6. Takeoff (climb via velocity setpoint) -------------------
        target_alt_rel = self.get_parameter("takeoff_alt").get_parameter_value().double_value
        climb_speed    = self.get_parameter("climb_speed").get_parameter_value().double_value
        target_abs_alt = self.initial_alt + target_alt_rel

        self.get_logger().info(
            f"Climbing to {target_alt_rel:.1f} m (absolute {target_abs_alt:.2f} m) …"
        )
        ...
        while rclpy.ok():
            ...
            if current_z >= target_abs_alt - 0.15:
                self.publish_vel(vz=0.0)
                break

            # Keep re-asserting OFFBOARD while climbing
            if self.state.mode != "OFFBOARD":
                self.set_mode("OFFBOARD")

            self.publish_vel(vz=climb_speed)
